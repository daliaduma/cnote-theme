...
// DOCUMENTATION
// COMPILE / WATCH SCSS FILES
// THIS WILL BE REMOVED FROM GULPFILE.JS
...
gulp.task('doc-html', function() {
    ...
    var files = docs.map(function (element) {
        return gulp.src('src/'+element+ '/**/*.html')
            .pipe(swig({defaults: { cache: false }}))
...
gulp.task('doc-scss', function() {
    return gulp.src("src/documentation/scss/**/*.scss")
        .pipe(sass().on('error', sass.logError ))
        .pipe(autoprefixer('last 2 versions'))
...
gulp.task("doc-watch", function () {
    gulp.watch("src/documentation/scss/**/*.scss", ['doc-scss']);
    gulp.watch("src/documentation/**/*.html", ['doc-html']);
    gulp.watch('documentation'+'/**/*.html').on('change', reload);
})
gulp.task('doc-images', function () {
    return gulp.src('src/documentation/images/*')
        .pipe(gulp.dest('./documentation/images/'));
});

gulp.task("docstart", ['doc-html', 'doc-scss', 'doc-js', 'doc-watch', 'doc-images' ]);
